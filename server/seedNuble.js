...
async function seed() {
  try {
    ...
    // Clear previous nuble events if any
    await Panorama.deleteMany({ region: "Ñuble" });
    console.log('Deleted old Ñuble events');
    
    const rawData = fs.readFileSync('nuble_raw.txt', 'utf8');
    ...
    for(const line of lines) {
      ...
      eventos.push({
        ...
        lugar: lugar,
        titulo: titulo,
        descripcion: desc || 'Panorama en Ñuble',
    ...
    console.log("Inserted " + eventos.length + " events for Ñuble");
    process.exit(0);
  } catch (err) {
    console.error(err);
    process.exit(1);
