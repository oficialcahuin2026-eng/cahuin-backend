...
text = """Hasta el 12 de julio	Expo Creadoras Futrono	Paseo Alessandri, Futrono	Ferias
...
def format_iso(fecha_str):
    # Quick dummy conversion, assume July 2026 for everything to be valid.
    import re
    match = re.search(r'(\d+)\s+de\s+(julio|Julio)', fecha_str)
    if match:
        day = int(match.group(1))
        return f"2026-07-{day:02d}T20:00:00.000Z"
    return "2026-07-15T20:00:00.000Z"

for i in range(0, len(lines), 2):
    row = lines[i].split('\t')
    if len(row) >= 4:
        fecha, evento, lugar, cat = [r.strip() for r in row[:4]]
        desc = lines[i+1].strip() if i+1 < len(lines) else ''
        events.append({
            '_id': f'evt_mock_{len(events)}',
            'titulo': evento,
            'lugar': lugar,
            'fecha': format_iso(fecha),
            'fechaString': fecha,
            'categoria': cat,
            'descripcion': desc,
            'esOficial': True,
        ...
        if 'concepción' in lugar_lower or 'talcahuano' in lugar_lower or 'biobío' in lugar_lower or 'lota' in lugar_lower or 'coronel' in lugar_lower or 'penco' in lugar_lower or 'lebu' in lugar_lower or 'arauco' in lugar_lower or 'san pedro' in lugar_lower or 'los ángeles' in lugar_lower or 'huépil' in lugar_lower or 'chiguayante' in lugar_lower or 'tomé' in lugar_lower or 'yumbel' in lugar_lower or 'cañete' in lugar_lower:
             region = 'Bío Bío'
        else:
             region = 'Los Ríos'
             
        events[-1]['region'] = region

with open('C:\\Users\\gonza\\OneDrive\\Desktop\\cahuin\\src\\data\\mockPanoramas.js', 'w', encoding='utf-8') as f:
    f.write('export const mockPanoramas = ' + json.dumps(events, indent=2, ensure_ascii=False) + ';\n')
